...
if [ -d "$WLP_SOURCE" ]; then
    ...
    sudo chown -R neil:www-data $FOLDER_PATH
    sudo chmod 770 $FOLDER_PATH -R
    echo "Copied WordPress files to $FOLDER_PATH and set permissions"
else
    echo "WordPress source files not found at $WLP_SOURCE"
    exit 1
fi
